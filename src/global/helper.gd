...
static func get_long_description_type(type: Enums.EffectType, strength: float = 0.0):
	match type:
		...
		Enums.EffectType.Water:
			return get_long_description("water", strength)
		_:
			return ""
...
static func get_long_description(effect_name: String, strength: float = 0.0):
	match effect_name:
		...
		"regrow_strength":
			return "[color=gold]Regrow " + str(strength) + "[/color]: When harvested, seed is re-planted with +" + str(strength) + Helper.mana_icon() + " on the same tile"
		"add_recurring":
